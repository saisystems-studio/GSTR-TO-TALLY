...
`powershell -ExecutionPolicy Bypass -File .\build-exe.ps1 -Origin "https://saitallyexpert.in"`

The release runner needs Python and Inno Setup 6 (`ISCC.exe`). The one command builds the PyInstaller folder, compiles a genuine Inno Setup installer, and produces `local-tally-agent/release/GSTR2TallyConnectorSetup.exe`. Upload that exact file to `/var/www/gstr2tally/downloads/GSTR2TallyConnectorSetup.exe` with the web release. The installer is the only customer-facing artifact. It installs the complete PyInstaller distribution under Program Files, starts the connector after installation, and registers it for future Windows sign-ins.
